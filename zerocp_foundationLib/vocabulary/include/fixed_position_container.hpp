...
#ifndef ZEROCP_FOUNDATION_VOCAB_FIXED_POSITION_CONTAINER_HPP
#define ZEROCP_FOUNDATION_VOCAB_FIXED_POSITION_CONTAINER_HPP
...
struct BestFittingUnsigned
{
    using type = std::conditional_t<(CAPACITY <= std::numeric_limits<uint8_t>::max()),
                                    uint8_t,
                                    std::conditional_t<(CAPACITY <= std::numeric_limits<uint16_t>::max()),
                                                       uint16_t,
                                                       std::conditional_t<(CAPACITY <= std::numeric_limits<uint32_t>::max()),
                                                                          uint32_t,
                                                                          uint64_t>>>;
};

template <>
struct BestFittingUnsigned<0>
{
    using type = uint8_t;
};

template <uint64_t CAPACITY>
using BestFittingUnsigned_t = typename BestFittingUnsigned<CAPACITY>::type;

inline void enforce(bool condition, const char* message) noexcept
{
    if (!condition)
    {
        std::fputs("FixedPositionContainer: ", stderr);
...
#ifndef IOX_ENFORCE
#define IOX_ENFORCE(condition, message) ::ZeroCP::detail::enforce((condition), (message))
#endif
...
class FixedPositionContainer final
{
  ...
  public:
    ...
    static_assert(CAPACITY > 0U, "FixedPositionContainer capacity must be greater than zero.");
    static_assert(std::is_nothrow_destructible_v<T>, "T must be nothrow destructible.");

    using IndexType = detail::BestFittingUnsigned_t<CAPACITY>;
    ...
    FixedPositionContainer(const FixedPositionContainer& rhs) noexcept(std::is_nothrow_copy_constructible_v<T>);
    FixedPositionContainer(FixedPositionContainer&& rhs) noexcept(std::is_nothrow_move_constructible_v<T>);

    FixedPositionContainer& operator=(const FixedPositionContainer& rhs) noexcept(std::is_nothrow_copy_constructible_v<T>);
    FixedPositionContainer& operator=(FixedPositionContainer&& rhs) noexcept(std::is_nothrow_move_constructible_v<T>);
    ...
    /// failed
    Iterator insert(const T& data) noexcept(std::is_nothrow_copy_constructible_v<T>);
    ...
    Iterator emplace(Targs&&... args) noexcept(std::is_nothrow_constructible_v<T, Targs&&...>);
    ...
    class IteratorBase
    {
      public:
        ...
        IteratorBase& operator++() noexcept
        {
            ...
            {
                m_index = m_container.get().m_nextUsed[m_index];
            }
            return *this;
        }
        ...
        [[nodiscard]] Value& operator*() const
        {
            ...
            return *m_container.get().elementAt(m_index);
        }
  ...
  private:
    ...
    Iterator emplaceImpl(Args&&... args) noexcept(std::is_nothrow_constructible_v<T, Args&&...>);

    void initializeFreeList() noexcept;

    Iterator iteratorFromIndex(IndexType index) noexcept;
    ConstIterator iteratorFromIndex(IndexType index) const noexcept;

    T* elementAt(IndexType index) noexcept;
    const T* elementAt(IndexType index) const noexcept;

    void destroyElement(IndexType index) noexcept;
    IndexType acquireSlot() noexcept;
    void releaseSlot(IndexType index) noexcept;

    void addUsedSlot(IndexType index) noexcept;
    IndexType removeUsedSlot(IndexType index) noexcept;

    IndexType findIndexFromPointer(const T* ptr) const noexcept;

  private:
    std::array<std::aligned_storage_t<sizeof(T), alignof(T)>, CAPACITY> m_storage{};
    std::array<SlotStatus, CAPACITY> m_status{};
    std::array<IndexType, CAPACITY> m_nextUsed{};
    std::array<IndexType, CAPACITY> m_nextFree{};
    IndexType m_size{0};
...
#include "../detail/fixed_position_container.inl"

#endif // ZEROCP_FOUNDATION_VOCAB_FIXED_POSITION_CONTAINER_HPP
